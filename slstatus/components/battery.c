 ...
 */
     ...
     battery_state(const char *bat)
   {
    ...
    static const char *charging_icons[] = { "", "", "", "", "" }; // Unicode symbols for charging
    static size_t icon_index = 0; // Track the current icon index
    size_t i;
    ...
    // If charging, return an alternating icon
    if (!strcmp(state, "Charging")) {
        const char *icon = charging_icons[icon_index];
        icon_index = (icon_index + 1) % LEN(charging_icons); // Move to the next icon
        return icon;
    }

    // If discharging, return an icon based on the battery percentage
